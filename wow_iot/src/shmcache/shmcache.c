...
#include "shmcache/shm_object_pool.h"
#include "shmcache/shm_striping_allocator.h"
#include "shmcache/shm_op_wrapper.h"
#include "shmcache/shmopt.h"
#include "shmcache/shm_list.h"
#include "shmcache/shm_lock.h"
...
static int parse_bytes(const char *pStr, const int default_unit_bytes, int64_t *bytes)
{
	char *pReservedEnd;
    int result;
...
static int64_t shmcache_parse_bytes(Ini_PT iniContext,
        const char *config_filename, const char *name, int *result)
{
    char *value;
    int64_t size;
    *result = wow_ini_get_string(iniContext,name,&value);
    if (value == NULL) {
        logError("file: "__FILE__", line: %d, "
...
static int64_t shmcache_parse_bytes_with_default(Ini_PT iniContext,
		...
		const char *config_filename)
{
    ...
    Ini_PT iniContext = wow_ini_init(config_filename);
    CHECK_RET_VAL_P(iniContext,-1,"wow_ini_init failed!\n");

    do {
        result = wow_ini_get_string(iniContext,":type",&type);
        ...
        result = wow_ini_get_string(iniContext,":filename",&filename);
        ...
        snprintf(config->filename, sizeof(config->filename),"%s", filename);

        config->max_memory = shmcache_parse_bytes(iniContext, config_filename, ":max_memory", &result);
        if (result != 0) {
            break;
        }

        config->min_memory = shmcache_parse_bytes_with_default(iniContext,config_filename, ":min_memory", 0, &result);
        config->segment_size = shmcache_parse_bytes(iniContext,config_filename, ":segment_size", &result);
        if (result != 0) {
            break;
        }
        ...
        result = wow_ini_get_int(iniContext,":max_key_count",&config->max_key_count );
        ...
        config->max_value_size = shmcache_parse_bytes(iniContext,config_filename, ":max_value_size", &result);
        if (result != 0) {
            break;
        }

        result = wow_ini_get_string(iniContext,":hash_function",&hash_function);
        if (result !=0 || hash_function == NULL || *hash_function  == '\0') {
            config->hash_func = wow_hash_data;
        } else {
            void *handle;
            ...
            config->hash_func = (hash_func_t)dlsym(handle, hash_function);
            if (config->hash_func == NULL) {
                snprintf(new_funcname, sizeof(new_funcname),
                        "fc_%s", hash_function);
                config->hash_func = (hash_func_t)dlsym(handle, new_funcname);
                if (config->hash_func == NULL) {
                    logError("file: "__FILE__", line: %d, "
        ...
        result = wow_ini_get_int(iniContext,":value_policy.avg_key_ttl",&config->va_policy.avg_key_ttl );
        config->va_policy.discard_memory_size = shmcache_parse_bytes(iniContext, config_filename,":value_policy.discard_memory_size", &result);
        if (result != 0) {
            break;
        }
        result = wow_ini_get_int(iniContext,":value_policy.max_fail_times",&config->va_policy.max_fail_times);
        result = wow_ini_get_int(iniContext,":lock_policy.trylock_interval_us",&config->lock_policy.trylock_interval_us);
        ...
        result = wow_ini_get_int(iniContext,":value_policy.sleep_us_when_recycle_valid_entries",&config->va_policy.sleep_us_when_recycle_valid_entries );
       
        result = wow_ini_get_int(iniContext,":lock_policy.detect_deadlock_interval_ms",&config->lock_policy.detect_deadlock_interval_ms);
        ...
        result = wow_ini_get_int(iniContext,":recycle_key_once",&config->recycle_key_once );
        if (config->recycle_key_once <= 0) {
            config->recycle_key_once = -1;
        }
        result = wow_ini_get_bool(iniContext,":recycle_valid_entries",&config->recycle_valid_entries);

        int log_level = 0;
        result = wow_ini_get_int(iniContext,":log_level",&log_level);
        printf("log_level:%d\n",log_level);
        wow_log_set_module_level(SHMCACHE_LOG_TAG,log_level);
    } while (0);

    wow_ini_exit(&iniContext);
    return result;
}
