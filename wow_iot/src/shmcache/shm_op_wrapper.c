...
static int writeToFile(const char* filename,const char *buff, const int file_size)
{
    int len = 0;

    File_T* file = wow_hfile_open(FILE_IO,filename,FILE_CREATE|FILE_RDWR);
    CHECK_RET_VAL_P(file,-1,"wow_hfile_open failed!\n");

    len = wow_hfile_write(file,buff,file_size);
    wow_hfile_close(file);

    return (len==file_size)?0:-1;
}
