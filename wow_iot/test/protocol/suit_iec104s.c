...
static void* test_iec104s_signal_spon(void* handle)
{
	ModDebug_D(MOD_TAG,"suit_iec104s----test_iec104s_signal_spon\n");
	...
	while(1){
		...
		wow_iec104_s_build_yx_spon(handle,node,2);
		sleep(5);
		...
		wow_iec104_s_build_yx_spon(handle,node,1);
		sleep(5);
...
static void* test_iec104s_detect_spon(void* handle)
{
	ModDebug_D(MOD_TAG,"suit_iec104s----test_iec104s_detect_spon\n");
	...
	while(1){
		...
		wow_iec104_s_build_yc_spon(handle,node,2);
		sleep(5);
		...
		wow_time_get_cp56(&node[0].value.time);
		wow_iec104_s_build_yc_spon(handle,node,1);
		sleep(5);
...
static int iec104s_set_value(void* priv,DevNodeInfo_T* node,DevNodeVal_T* val)
{	
	int i = 0;
	...
	for(i = 0; i < ARRAY_SIZE(gt_test_call); i++){
		...
		printf("iec104s_set_value-----------addr:%d val.u32:%d val.f:%f\n",node->addr,val->val.u32,val->val.f);
...
static int iec104s_get_value(void* priv,DevNodeInfo_T* node,DevNodeVal_T* val)
{	
	int i = 0;
...
static int iec104s_set_service(void* priv,uint8_t ser_id,void* arg)
{
...
TEST test_iec104s_fun(void)
{
	int ret = 0;
	ProtoEthParam_T link;


	ModDebug_I(MOD_TAG,"suit_iec104s----test_iec104s_fun\n");

	ret =  wow_eth_getLocalIp(link.host);
	GREATEST_ASSERT(ret == 0);
	link.port = SUIT_TEST_IEC104S_PORT;

	Iec104s_T* iec104s = wow_iec104_s_init(0x0001,&link);
	GREATEST_ASSERT(iec104s);

	ret = wow_iec104_s_set_node_info(iec104s,gt_test_call,ARRAY_SIZE(gt_test_call));
	GREATEST_ASSERT(ret == 0);
	ret = wow_iec104_s_set_value_handle(iec104s,iec104s_set_value,NULL);
	GREATEST_ASSERT(ret == 0);
	ret = wow_iec104_s_get_value_handle(iec104s,iec104s_get_value,NULL);
	GREATEST_ASSERT(ret == 0);
	ret = wow_iec104_s_set_service_handle(iec104s,iec104s_set_service,NULL);
	GREATEST_ASSERT(ret == 0);

	ret = wow_iec104_s_start(iec104s);
	GREATEST_ASSERT(ret == 0);
	

	wow_thread_create(MOD_TAG,test_iec104s_signal_spon, iec104s);
	wow_thread_create(MOD_TAG,test_iec104s_detect_spon, iec104s);
